 ...
 *
 * Adapted: start_dns_server_on_netif() added to accept explicit esp_netif_t *
 * for use with esp_wifi_remote where ifkey-based lookup may not work.
 */
...
static int parse_dns_request(char *req, size_t req_len, char *dns_reply, size_t dns_reply_max_len, dns_server_handle_t h)
{
    ...
    for (int qd_i = 0; qd_i < qd_count; qd_i++) {
        ...
        uint16_t qd_class = ntohs(question->class_val);
        ...
        if (qd_type == QD_TYPE_A) {
            ...
            answer->class_val = htons(qd_class);
...
{
    if (ap_netif == NULL) {
        ESP_LOGE(TAG, "start_dns_server_on_netif: ap_netif is NULL");
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "start_dns_server_on_netif: failed to get IP info: %s", esp_err_to_name(err));
        return err;
    }

    ESP_LOGI(TAG, "DNS server will redirect all queries to " IPSTR, IP2STR(&ip_info.ip));

    dns_server_config_t config = {
        .num_of_entries = 1,
        .item = {
            {
                .name = "*",        // wildcard — answer ALL DNS queries
                .if_key = NULL,     // use static IP below
                .ip = ip_info.ip,
            }
        }
    };

    dns_server_handle_t handle = start_dns_server(&config);
    if (handle == NULL) {
        ESP_LOGE(TAG, "start_dns_server_on_netif: failed to start DNS server");
        return ESP_FAIL;
    }
